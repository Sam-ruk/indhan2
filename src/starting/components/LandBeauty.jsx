...
function LandBeauty () {
    ...
    useEffect(() => { 
        fetch(`18.142.128.26:4000/buyers`,{method: 'GET'})
          .then(async (resp)=>{
            var x = await resp.json();
            setBuyer(x.count);
          });
        fetch(`18.142.128.26:4000/sellers`,{method: 'GET'})
          .then(async (resp)=>{
            var x = await resp.json();
            setSeller(x.count);
          });
        fetch(`18.142.128.26:4000/ethanol`,{method: 'GET'})
          .then(async (resp)=>{
            var x = await resp.json();
            setEthanol(x.count);
          });
        fetch(`18.142.128.26:4000/diesel`,{method: 'GET'})
          .then(async (resp)=>{
            var x = await resp.json();
            setDiesel(x.count);
          });
        fetch(`18.142.128.26:4000/cng`,{method: 'GET'})
          .then(async (resp)=>{
            var x = await resp.json();
            setCng(x.count);
          });
